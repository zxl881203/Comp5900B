...
Truck::Truck(Prey* prey){
	...
	printf("lookatv %f %f %f  \n", prey->lookAtVector.x, prey->lookAtVector.y, prey->lookAtVector.z);
...
int Truck::updateState(int time){
	...
	*/
	...
	if (time % 50 == 0)printf("prey 's lookat vector is %d  %d \n", this->lookAtVector.x, this->lookAtVector.z);
	...
	if(distance <= DISTANCE_THRESHOLD&&directionDifference<ANGLE_THRESHOLD){// 1.if the distance between the shadowPrey and its wrapping prey is with in the distance
		...
		this->currentTime = 0;// std::time(0) * 1000;
