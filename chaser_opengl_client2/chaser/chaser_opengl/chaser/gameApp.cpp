...
//net work related 
ShadowPrey *gameApp::shadowPrey = NULL;
ShadowChaser *gameApp::shadowChaser = NULL;
ShadowChaser *gameApp::shadowChaserYellowKia = NULL;
...
gameApp::gameApp(void) 
{
	...
	myApp = this;
	this->nc = new NetClient(this);//initialize the net client
}
...
int gameApp::initGraphics(int argc, char** argv, int winWidth, int winHeight, int winPosx, int winPosy)
{
	...
	// instuct openGL what window size ot use
	glutInitWindowSize(500, 500);		// no magic numbers
...
int gameApp::initGame(void)
{
	...
	//I do not think the truck object is of much use at the momnet
	//so I comment it out at the momnet
	...
	//here  we set the initial position of the prey(user avatar)
	//parameters are: location, lookAtVector, UpVector 
	...
	shadowPrey = new ShadowPrey(preyRedKia);
	//shadowPrey related(net work--deadReckoning model)
	shadowPrey->createShaderProg("general.vert", "general.frag");
	// load the gemoetry
	shadowPrey->loadModelOBJ("kia\\kia_rio.obj", &shadowPrey->mVtxBuf, &shadowPrey->mNumVtx, &shadowPrey->mIndBuf, &shadowPrey->mNumInd);
	// load the textures
	shadowPrey->loadTexture("kia\\rio_red.bmp");
	// set the initial position attributes - align the object wth the z-axis if needed
	shadowPrey->setScale((float) 0.2, (float) 0.2, (float) 0.2);
	shadowPrey->mYaw = 90;		// orient it so that it faces that +zaxis
	gameDynamicEntities.push_back(shadowPrey);
	...
	house2->setPositionOrientation(Vector3f(-5,0,-0.2), Vector3f(-1,0,1),Vector3f(0,1,0));
	...
	*/
	...
	th = new TargetHouse();
	th->createShaderProg("generalRed.vert", "generalRed.frag");
	...
	cam->setPerspectiveView(45, 0.5,(float) 0.2,1000);
...
//			refractFlag = (refractFlag == 0) ? 1 : 0;
		...
		case 'i':
					...
					//*preyPers = *(preyRedKia->cam);
					
			break;
		//case 'k':

					//collide, backward one unit
					//preyRedKia->moveForward(1);
		...
		case 'l':
			//right
				preyRedKia->yaw(-10);
				//change the peryPers according to the movement of the prey
				//*preyPers = *(preyRedKia->cam);
